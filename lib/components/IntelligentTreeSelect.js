...
class IntelligentTreeSelect extends _react.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    ...
    for (const valueElement of values) {
      ...
      if (opt) modifiedSelectedOptions.push(opt);else modifiedInitialSelection.push(key);
    }
  ...
  _finalizeSelectedOptions(addedOptions, parsedOptions) {
    const foundOptions = [];
    let previouslySelected = (0, _Utils.sanitizeArray)(this.state.initialSelection);
    ...
    this.setState({
      initialSelection: previouslySelected
    });
  }
